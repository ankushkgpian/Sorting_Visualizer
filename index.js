...
function intro() {
  ...
  setTimeout(function() {
    ...
    // messageElement.classList.remove('intro-msg');
    ...
    for (let j = 0; j < array.length - i - 1; j++) {
      if (array[j] > array[j + 1]) {
        for (let k = 0; k < bars.length; k++) {
          if (k !== j && k !== j + 1) {
            bars[k].style.backgroundColor = "#457B9D";
          }
        }
        let temp = array[j];
        array[j] = array[j + 1];
...
async function swap(items, leftIndex, rightIndex, bars) {
  ...
  bars[leftIndex].style.backgroundColor = "#F1FAEE";
  bars[rightIndex].style.height = items[rightIndex] * heightFactor + "px";
  bars[rightIndex].style.backgroundColor = "#F1FAEE";
  await sleep(speedFactor);
}
async function partition(items, left, right) {
  ...
  bars[pivotIndex].style.backgroundColor = "#457B9D";

  for (let i = 0; i < bars.length; i++) {
    if (i != pivotIndex) {
      bars[i].style.backgroundColor = "#457B9D";
    }
  }
...
async function HeapSort(array) {
  ...
  for (let k = 0; k < bars.length; k++) {
    bars[k].style.backgroundColor = "#F1FAEE";
    await sleep(speedFactor);
  }
  return array;
}
...
async function mergeSort(arr) {
  ...
  while (i < left.length && j < right.length) {
    ...
    if (k + arr.length < bars.length) {
      ...
      bars[k + arr.length].style.backgroundColor = "#F1FAEE";
    }
    await sleep(speedFactor);
